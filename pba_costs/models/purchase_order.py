...
class PurchaseOrder(models.Model):
    ...
    def action_pba_update_sale_prices(self):
        ...
        for line in self.order_line:
            delta = line._pba_build_template_update_vals_from_line()
            if not delta:
                continue
        ...
        if not vals_by_tmpl:
            raise UserError(
                _("No hay líneas de producto para actualizar en esta orden.")
            )
        for tid, vals in vals_by_tmpl.items():
            self.env["product.template"].browse(tid).sudo().write(vals)
        for line in lines_updated:
            line.write(line._pba_baseline_vals_from_line())
        return {
            "type": "ir.actions.client",
            "tag": "display_notification",
            "params": {
                "title": _("Precios y costos"),
                "message": _(
                    "Se actualizaron los porcentajes de costo y el precio de venta "
                    "de %(count)s producto(s).",
